...
namespace OpenWeatherApp.Api.OpenWeather.Endpoints
{
    public interface IOpenWeatherGeolocationApi
    {
        ...
        Task<ApiResponse<IEnumerable<LatLonLocation>>> GetLocationByLatLon(string latitude, string longitude, string appId,
            CancellationToken cancellationToken = default);
        
        //http://api.openweathermap.org/geo/1.0/direct?q={city name},{state code},{country code}&limit={limit}&appid={API key}
        [Get("/direct?q={cityName}," + UnitedStatesCode)]
        Task<ApiResponse<IEnumerable<ZipCodeLocation>>> GetLocationByCityName(string zipCode, string countryCode, string appId,
            CancellationToken cancellationToken = default);
    }
}